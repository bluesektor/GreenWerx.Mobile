...
import {GeoHelperFunctions} from '../../geo/geo.helper.functions';
...
@Component({
  selector: 'app-events-page-lists',
  templateUrl: './events.page.lists.component.html',
  styleUrls: ['./events.page.lists.component.scss']
  
})
export class EventsPageListsComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    public viewContainerRef: ViewContainerRef
     ...
     this.router.routeReuseStrategy.shouldReuseRoute = function() {
          // tslint:disable-next-line:max-line-length
          console.log('events.page.lists.component.ts constructor  this.router.routeReuseStrategy.shouldReuseRoute this.virtualScroll:', self.virtualScroll );
    ...
    console.log('events.page.lists.component.ts constructor');
    ConsoleColors.colorLog('events.page.lists.component.ts constructor', 'info');
  }
...
*/
  ...
  addBottomSpacer() {
    ...
    if ( this.itemCount === 0  || this.itemCount > this.cache.events.length
      ) {
      console.log('events.page.lists.component.ts addBottomSpacer return without adding');
      return;
    }
    ...
    this.cache.events.push(blankNode);

    const blankNode2 = new Node();
    blankNode2.Name = '';
    blankNode2.IsBlank = true;
    blankNode2.UUIDType = 'blank';
    this.cache.events.push(blankNode2);
    this.bottomSpacerAdded = true;
    console.log('events.page.lists.component.ts addBottomSpacer added');
  }
  ...
  async deleteItem(slidingItem: HTMLIonItemSlidingElement, item: any, title: string) {
    console.log('events.page.lists.component.ts deleteItem');
           ...
           handler: () => {
             ...
             if ( ObjectFunctions.isValid( this.eventService.Dashboard  ) === true ) {
               for (let i = 0; i < this.eventService.Dashboard.Events.length; i++) {
                 console.log('events.page.lists.component.ts remove favorite searching x:', this.eventService.Dashboard.Events[i]);
              ...
              slidingItem.close();
              console.log('events.page.lists.component.ts deleteItem item.UUIDType:', item.UUIDType);
  ...
  doRefresh(event: Refresher) {
    console.log('events.page.lists.component.ts doRefresh  updateList');
    this.itemCount = 0;
  ...
  async loadEvents() {
      console.log('events.page.lists.component.ts loadEvents    sessionService.CurrentSession :', this.sessionService.CurrentSession );
      const tidx = Timer.start('events.page.lists.component.ts loadEvents');
      const filter = this.filterService.getFilter(this.viewType);
    //  if (   this.isAdmin === true) {
     //   filter.IncludePrivate = true;
     // }
      console.log('events.page.lists.component.ts loadEvents filter :', filter);
      this.isLoading = true;
      ...
      await this.eventService.getEvents(filter)
        .subscribe(response => {
          ...
          this.noDataMessage = '';
          console.log('events.page.lists.component.ts loadEvents   response:', response);
        ...
        const data = response as ServiceResult;
        console.log('events.page.lists.component.ts loadEvents   data:', data);
        if (data.Code !== 200) {
        ...
        for ( let i = 0; i < data.Result.length; i++) {
          this.cache.events.push(data.Result[i]);
        }
      // this.cache.events = this.cache.events.concat(data.Result); // optimization_change

       console.log('events.page.lists.component.ts loadEvents   this.cache.events:', this.cache.events);

       console.log('eventspage.TS loadEvents test distance this.sessionService.CurrentSession.Profile.:' ,
       this.sessionService.CurrentSession.Profile);
        ...
        console.log('eventspage.TS loadEvents    this.cache.events:',  this.cache.events);

           // App logic to determine if all data is loaded
        // and disable the infinite scroll
        if (
          ObjectFunctions.isValid(this.infinitListEvent) &&
          this.cache.events.length >= this.itemCount
        ) {
      ...
      err => {
        this.isLoading = false;
        this.noDataMessage = '';
        console.log('eventspage.TS loadEvents  err:', err);
  ...
  async loadFavorites() {
    console.log('events.page.lists.component.ts onLogin loadFavorites :');
    this.cache.events = [];
    ...
    this.favoritesService.getFavorites(type, null).subscribe(sessionResponse => {
      ...
      console.log('events.page.lists.component.ts  loadFavorites.data :', data);
      if (data.Code !== 200) {
        this.messages.publish('api:err', data);
        return false;
      }
      ...
      for (let i = 0; i < data.Result.length; i++) {
        ...
        this.cache.events.push(data.Result[i].Item);
      }
      // this.cache.events = this.cache.events.concat(data.Result);
      console.log('events.page.lists.component.ts loadFavorites this.events:', this.cache.events);
      // Re-render Virtual Scroll List After Adding New Data
      if (ObjectFunctions.isValid(this.virtualScroll) === true) {
        this.virtualScroll.checkEnd();
      }
  ...
  async loadHosts() {
    console.log('events.page.lists.component.ts loadHosts    sessionService.CurrentSession :', this.sessionService.CurrentSession );
    const tidx = Timer.start('events.page.lists.component.ts loadEvents');
    const filter = this.filterService.getFilter(this.viewType);
  //  if (   this.isAdmin === true) {
   //   filter.IncludePrivate = true;
   // }
    ...
    console.log('events.page.lists.component.ts loadHosts filter :', filter);
    this.isLoading = true;
    this.noDataMessage = '';
    await this.accountService.getAllAccounts(filter).subscribe(response => {
        this.noDataMessage = '';
        console.log('events.page.lists.component.ts loadHosts   response:', response);
      if (ObjectFunctions.isValid(this.refresherRef) === true) {
        this.refresherRef.complete();
      }
      if (ObjectFunctions.isValid(this.infinitListEvent)) {
        ...
        const data = response as ServiceResult;
      console.log('events.page.lists.component.ts loadHosts   data:', data);
        if (data.Code !== 200) {
        this.isLoading = false;
          this.messages.publish('api:err', data);
          return;
    ...
    // this.cache.events = this.cache.events.concat(data.Result); // optimization_change
      console.log('host-list.ts this.accountService.getAllAccounts   this.cache.events:',  this.cache.events   );
     // tslint:disable-next-line:max-line-length
     console.log('eventspage.TS loadEvents test distance this.sessionService.CurrentSession.Profile.:' , this.sessionService.CurrentSession.Profile);
      // Re-render Virtual Scroll List After Adding New Data
      if (ObjectFunctions.isValid(this.virtualScroll) === true) {
        this.virtualScroll.checkEnd();
      }
      ...
      console.log('eventspage.TS loadHosts    this.cache.events:',  this.cache.events);
      // App logic to determine if all data is loaded
      // and disable the infinite scroll
      if (
        ObjectFunctions.isValid(this.infinitListEvent) &&
    ...
    err => {
      this.isLoading = false;
      this.noDataMessage = '';
      console.log('eventspage.TS loadHosts  err:', err);
  ...
  loadMoreInViewPort(event: any) {
    ...
    filter.StartIndex = this.cache.events.length;
    ...
    if (index < 0) {
      ...
      this.filterService.setFilter(filter);
        ...
        console.log('10. events.page.lists.component.ts loadMoreInViewPort  updateList');
  ...
  ngOnInit() {
    console.log('events.page.lists.component.ts ngOnInit started');
    ...
    console.log('events.page.lists.component.ts ngOnInit this.isLoggedIn', this.isLoggedIn);
    console.log('events.page.lists.component.ts ngOnInit this.sessionService', this.sessionService);
    ...
    this.localSettings.getValue(LocalSettings.ViewType, 'EVENT').then(typeRes => {
      console.log('events.page.lists.component.ts localSettings res:', typeRes);
  ...
  ngAfterViewInit(): void {
    console.log('events.page.lists.component.ts ngAfterViewInit update list ');
  }

  initializeView() {
    console.log('events.page.lists.component.ts initializeView');
  }

  ionViewDidEnter() {
    console.log('events.page.lists.component.ts ionViewDidEnter');
  }

  async removeFavoriteItem(slidingItem: HTMLIonItemSlidingElement, item: any, title: string) {

    console.log('events.page.lists.component.ts removeFavoriteItem slidingItem:' ,  slidingItem);
          ...
          handler: () => {
            ...
            this.favoritesService.removeFavorite(item.UUID).subscribe(
              response => {
                ...
                console.log(
                  'events.page.lists.component.ts removeFavoriteItem  updateList'
                );
                this.updateList(
                  true,
                  this.segment,
  ...
  public updateList(
    ...
    viewType: string,
    queryText: string  ) {
      console.log('events.page.lists.component.ts updateList A this.segment:', segment); 
    this.showDistance = this.sessionService.isLocationSet();
    console.log('events.page.lists.component.ts  updateList B this.segment:', segment);
    // reset before assigning parameters. If not the filters from events filter won't work
    if (resetArray === true) {
      this.cache.events = [];
      this.itemCount = 0;
      this.bottomSpacerAdded = false;
      // this.filterService.resetFilter(viewType);
      console.log('events.page.lists.component.ts updateList C this.segment:', segment);
    }
    // Close any open sliding items when the listItems updates
    console.log('events.page.lists.component.ts updateList D this.segment:', segment);
    this.segment = segment;
    this.viewType = viewType;
    this.queryText = queryText; // todo could change query text by viewType i.e. eventQueryText, profilesQueryText...

    console.log('events.page.lists.TS updateEvents  this.viewType:', this.viewType);
    this.isLoading = true;
