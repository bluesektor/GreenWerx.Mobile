...
export class EventsFilterPage implements AfterViewInit {
  ...
  applyFilters() {
    ...
    if (index < 0) {
      console.log('events-filter.ts applyFilters NO FILTER FOUND');
      return;
    }
    ...
    switch ( this.type) {
        case 'EVENT':
           screens = this.eventService.AvailableScreens.filter( c => c.Selected === true).map(function(c) {
            console.log('events-filter.ts applyFilters this.eventService.AvailableScreens.filter:', c);
            return c;
            });
        break;
        case 'HOST':
            screens =
                ...
                console.log('events-filter.ts applyFilters this.accountService.AvailableScreens.filter:', c);
                return c;
            });
        break;
      }
...
dismiss(filter: Filter) {
  ...
  initializeView() {
    console.log('A. events-filter.ts initializeView');
    this.loading = true;
    let sortBy =  'Name';
    let sortDir = 'ASC';

    console.log('C events-filter.ts initializeView this.type:', this.type);
    const index = this.filterService.getFilterIndex(this.type);
    if (index >= 0) {
      switch ( this.type) {
        case 'EVENT':
            console.log('C.1 events-filter.ts initializeView index:', index);
            console.log('C.1 events-filter.ts initializeView this.filterService:', this.filterService);
            console.log('C.1 events-filter.ts initializeView  this.filterService.Filters[index].:', this.filterService.Filters[index]);
          sortBy = 'StartDate';
    ...
    console.log('D events-filter.ts initializeView sortBy:', sortBy);

    this.sortByScreens.forEach( prop => {
          prop.Selected = false;
          console.log('D events-filter.ts initializeView prop.Value :', prop.Value );
          if (prop.Value.toLowerCase() === sortBy.toLowerCase()) {
            prop.Selected = true;
            console.log('E. events-filter.ts initializeView sortBy selected:', sortBy);
          }
        });
    this.loading = false;
  }
  ...
  ionViewDidEnter() {
    console.log('events-filter.ts ionViewDidEnter type:',  this.type);
  ...
  isUserInrole(roleName: string): boolean {
    console.log('isUserInRole  events-filter.ts ');
  ...
  async loadAccountCategories() {
    ...
    // NOTE: Name must match the property name
        ...
        if (accountFilter.Screens !== undefined && accountFilter.Screens.length > 0) {
          console.log('events-filter.ts cached screens  this.accountService.AvailableScreens:',  this.accountService.AvailableScreens);

          this.initializeView();
        } else {
          console.log('events-filter.ts loadAccountCategories');

          this.loading = true;
         await this.http.get( Api.url + 'Content/filters/account.web.json').subscribe(res => {
           if (res) {
             console.log('events-filter.ts loadAccountCategories res:', res);
             this.accountService.AvailableScreens = [];
             this.accountService.Categories = [];

             if (!Array.isArray(res)) {
               console.log('events-filter.ts loadAccountCategories !Array.isArray(res) returning');
               this.loading = false;
               return;
             }
  ...
  async loadEventCategories() {
    ...
    console.log('events-filter.ts loadEventCategories  eventFilter:', eventFilter);

    if (eventFilter.Screens !== undefined && eventFilter.Screens.length > 0) {
      // tslint:disable-next-line:max-line-length
      console.log('events-filter.ts loadEventCategories cached screens  this.eventService.AvailableScreens:',  this.eventService.AvailableScreens);
      // we want to use the cached screens so we can keep track when the user reloads this screen.
      this.initializeView();
    } else {
    console.log('events-filter.ts loadEventCategories');
   
    const file =  Api.url + 'Content/filters/event.web.json';
    console.log('events-filter.ts loadEventCategories file:', file);

    this.http.get(file)
    .subscribe(res => {
      if (res) {
        console.log('events-filter.ts loadEventCategories res:', res);
        this.eventService.AvailableScreens = [];
        this.eventService.Categories = [];

        if (!Array.isArray(res)) {
          console.log('events-filter.ts loadEventCategories !Array.isArray(res) returning');
          return;
        }
...
/*
  ...
  // NOTE: Name must match the property name
      ...
      if (this.eventService.EventFilter.Screens !== undefined && this.eventService.EventFilter.Screens.length > 0) {
        console.log('events-filter.ts cached screens  this.eventService.AvailableScreens:',  this.eventService.AvailableScreens);

        this.initializeView();
      } else {
        console.log('events-filter.ts loadEventCategories getEventCategories');
        this.loading = true;
        await this.eventService.getEventCategories().subscribe((response) => {
          console.log('events-filter.ts loadEventCategories getEventCategories response:', response);
          this.loading = false;
          ...
          this.eventService.AvailableScreens = [];
          this.eventService.Categories = [];
          console.log('events-filter.ts data.Result:', data.Result);
          if (ObjectFunctions.isValid(data.Result) === true) {
            data.Result.forEach(name => {
              if (!name) {
                return;
              }
              console.log('events-filter.ts data.Result.foreach:', name);
  ...
  onChangeRadioGroupSortBy(event: any) {
    console.log('events-filter.ts onChangeRadioGroupSortBy event:', event.detail.value);
    const index = this.filterService.getFilterIndex(this.type);
    console.log('events-filter.ts onChangeRadioGroupSortBy index:', index);
    if (index < 0) {
      return;
    }
  ...
  onToggleSortBy(event: any, sortby: any) {
    console.log('events-filter.ts onToggleSortBy ');
    if (this.loading === true ) {
      console.log('events-filter.ts onToggleSortBy EVENT sortby loading is true, returing');
      return false;
    }
    ...
    if ( 'distance' === sortby.Value.toLowerCase() ) {
      if (!navigator.geolocation ) {
        console.log('events-filter.ts getCurrentLocation returning');
        return;
      }

    // this.processing = true;
    navigator.geolocation.getCurrentPosition((resp) => {
    console.log('events-filter.ts.ts getCurrentLocation position:', resp);
    console.log('events-filter.ts.ts getCurrentLocation this.sessionService.CurrentSession:', this.sessionService.CurrentSession);
        ...
        console.log('events-filter.ts.ts getCurrentLocation setting filter location');
    ...
    // tslint:disable-next-line:max-line-length
    console.log('events-filter.ts onToggleSortBy  this.filterService.Filters[filterIndex].SortBy :',  this.filterService.Filters[filterIndex].SortBy );
    return (false);
  }

  radioBlur() {
    console.log('events-filter.ts radioBlur ');
  }

  radioFocus() {
    console.log('events-filter.ts radioFocus ');
  }
